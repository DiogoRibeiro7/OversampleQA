...
def test_batched_computation_limits_memory(monkeypatch):
    ...
    def spy(self, X1_, X2_, metric, batch_size, vectorized=None, **kwargs):
        ...
        return original(
            self, X1_, X2_, metric, batch_size, vectorized=vectorized, **kwargs
        )
    ...
    chunk_mem = optimizer.estimate_memory_gb(
        captured["batch_size"], result.shape[1], dtype=result.dtype
    )
...
def test_memory_validator_streams_when_memory_capped(monkeypatch):
    ...
    y = np.hstack(
        [np.zeros(len(X_majority), dtype=int), np.ones(len(X_minority), dtype=int)]
    )
    ...
    monkeypatch.setattr(
        MemoryEfficientValidator, "_streaming_validation", fake_streaming
    )
...
def test_validator_caching_reuses_results(tmp_path):
    ...
    result_first = validator.validate_oversampling(
        X, y, minority_label=1, oversampler=oversampler
    )
    result_second = validator.validate_oversampling(
        X, y, minority_label=1, oversampler=oversampler
    )
...
def test_validator_cache_invalidated_on_data_change(tmp_path):
    ...
    X_changed = X.copy()
    X_changed[0, 0] += 5.0
    validator.validate_oversampling(
        X_changed, y, minority_label=1, oversampler=oversampler
    )
...
def test_memory_validator_matches_standard():
    ...
    standard = validate_oversampling(
        X, y, minority_label=1, oversampler=oversampler, metric="euclidean"
    )
    memory_validator = MemoryEfficientValidator()
    optimized = memory_validator.validate_oversampling(
        X, y, minority_label=1, oversampler=oversampler, metric="euclidean"
    )
