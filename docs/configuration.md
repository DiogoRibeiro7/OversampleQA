...
OversampleQA can be configured via typed Python configs and via the
enhanced CLI config file.
...
> [!WARNING]
> **Without \`\`psutil\`\` installed, available memory is assumed to be
> 1 GB**, whatever the machine actually has. Batching is then more
> conservative than it needs to be, and throughput differs from an
> otherwise identical environment that has `psutil` — which makes
> performance reports hard to compare. The fallback is logged once at
> `INFO`.
